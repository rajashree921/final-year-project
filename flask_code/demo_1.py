...
new_inception, feature_model, pca, clf_SVM_new_inception, stopwords, countv, tformer, clf_Text, choice_matrix, choice_matrix_img, graph = (None,)*11

def load_data():
  global new_inception, feature_model, pca, clf_SVM_new_inception, stopwords, countv, tformer, clf_Text, choice_matrix, choice_matrix_img, graph
  ...
  feature_model = Model(inputs=new_inception.input, outputs=new_inception.get_layer('avg_pool').output)
...
def Final_textreader(img):
  ...
  Text1 = Text.copy()
  for word in Text1:
    if word in ['BESTSELLING','BESTSELLER','AUTHOR','YORK','TIMES']:
      Text.remove(word)
...
def predict():
  if request.method == 'POST' and request.files['file']:
    ...
    try:
      ...
      # os.remove(filename)
      return render_template('predict.html',img = filename, result_table = resultats.to_html(justify='left', border = 0), text = text_detected)
    except :
